...
{
    if (!opc_str) {
        LogMan::Msg::EFmt("[X86] Error: Null opcode string");
...
{
    if (!str) {
        LogMan::Msg::EFmt("[X86] Error: Null register string");
...
void set_x86_instr_opd_num(X86Instruction *instr, uint8_t num)
{
    ...
    if (num > MAX_X86_OPERANDS) {
        LogMan::Msg::EFmt("[X86] Warning: Operand number {} exceeds maximum, clamping to {}", num, MAX_X86_OPERANDS);
        num = MAX_X86_OPERANDS;
    }
    instr->opd_num = num;
}
...
void set_x86_instr_opd_reg(X86Instruction *instr, int opd_index, int regno, bool HighBits)
{
    if (!instr || opd_index < 0 || opd_index >= MAX_X86_OPERANDS) {
        LogMan::Msg::EFmt("[X86] Error: Invalid instruction or operand index in set_x86_instr_opd_reg");
        return;
    }
