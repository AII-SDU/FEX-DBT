...
{   
    ...
    if (cond < sizeof(arm_cc_table) / sizeof(arm_cc_table[0])) {
        instr->cc = arm_cc_table[cond];
    } else {
        LogMan::Msg::EFmt("[ARM] Error: Invalid condition code: {}", cond);
        instr->cc = ARM_CC_INVALID;
    }
}
...
{   
    if (!instr) {
        LogMan::Msg::EFmt("[ARM] Error: Null instruction pointer in set_arm_instr_opd_size");
        return;
    }
